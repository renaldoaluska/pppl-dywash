...
<?= $this->section('content') ?>
    ...
    <!-- ================================
         ...
         ================================ -->
    <div class="mb-8">
    <h3 class="text-lg font-semibold text-slate-700 mb-4 border-b pb-2">Pilih Alamat Pengiriman</h3>
    <?php if (!empty($addresses)): ?>
        <div class="grid gap-4">
            <?php foreach ($addresses as $address): ?>
                <label class="block cursor-pointer">
                    <input type="radio" name="address_id" value="<?= $address['address_id'] ?>"
                        class="peer hidden" <?= $address['is_primary'] ? 'checked' : '' ?> required>
                    <div class="border rounded-lg p-4 transition
                        border-slate-300 bg-white
                        peer-checked:border-blue-500 peer-checked:bg-blue-50">
                        <div class="flex justify-between items-center mb-1">
                            <span class="font-semibold text-slate-800"><?= esc($address['label']) ?></span>
                            <?php if ($address['is_primary']): ?>
                                <span class="text-xs bg-green-500 text-white rounded-full px-2 py-0.5">PRIMARY</span>
                            <?php endif; ?>
                        </div>
    ...
    <?php else: ?>
        <p class="text-slate-500">Anda belum memiliki alamat pengiriman.</p>
    <?php endif; ?>
</div>
