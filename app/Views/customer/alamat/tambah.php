...
<div class="bg-white rounded-xl p-6 shadow space-y-4 max-w-lg mx-auto">
    ...
    <h1 class="text-xl font-bold mb-4">Tambah Alamat</h1>
    ...
    <form action="/customer/profil/alamat/store" method="post" class="space-y-4">
        <?= csrf_field() ?>
        ...
        <div>
            ...
            <input type="text" name="label" id="label" class="w-full border border-slate-300 rounded-lg p-2" placeholder="Contoh: Rumah, Kantor" required value="<?= old('label') ?>">
        </div>

        <div>
            <label for="recipient_name" class="block text-sm font-medium text-slate-700">Nama Penerima</label>
            <input type="text" name="recipient_name" id="recipient_name" class="w-full border border-slate-300 rounded-lg p-2" required value="<?= old('recipient_name') ?>">
        </div>

        <div>
            <label for="phone_number" class="block text-sm font-medium text-slate-700">Nomor HP</label>
            <input type="text" name="phone_number" id="phone_number" class="w-full border border-slate-300 rounded-lg p-2" required value="<?= old('phone_number') ?>">
        </div>

        <div>
            <label for="address_detail" class="block text-sm font-medium text-slate-700">Detail Alamat</label>
            <textarea name="address_detail" id="address_detail" rows="3" class="w-full border border-slate-300 rounded-lg p-2" required><?= old('address_detail') ?></textarea>
        </div>

        <div class="grid grid-cols-2 gap-4">
    <div>
        <label for="latitude" class="block text-sm font-medium text-slate-700">Latitude</label>
        <input readonly type="text" name="latitude" id="latitude" class="w-full border border-slate-300 rounded-lg p-2 bg-slate-100 text-slate-500 opacity-80" required value="<?= old('latitude', '-7.250445') ?>">
    </div>
    <div>
        <label for="longitude" class="block text-sm font-medium text-slate-700">Longitude</label>
        <input readonly type="text" name="longitude" id="longitude" class="w-full border border-slate-300 rounded-lg p-2 bg-slate-100 text-slate-500 opacity-80" required value="<?= old('longitude', '112.768845') ?>">
    </div>
</div>
...
<div id="map" class="h-64 rounded-lg mt-4 border border-slate-300"></div>
        ...
        <div>
            <button type="submit" class="w-full bg-green-600 text-white font-bold py-3 rounded-lg hover:bg-green-700 transition-colors">Simpan Alamat</button>
        </div>
    </form>
</div>
<!-- Leaflet CSS & JS -->
